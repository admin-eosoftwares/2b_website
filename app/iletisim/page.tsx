...
export default function Iletisim() {
    ...
    // Client-side rate limiting (5 dakika)
    const isRateLimited = () => {
        const now = Date.now();
        const timeSinceLastSubmit = now - lastSubmitTime;
        const rateLimitWindow = 5 * 60 * 1000; // 5 dakika

        return timeSinceLastSubmit < rateLimitWindow;
    };
    ...
    const validateForm = () => {
        ...
        // Rate limiting check
        if (isRateLimited()) {
            newErrors.rateLimit = 'Çok sık mesaj gönderiyorsunuz. Lütfen 5 dakika sonra tekrar deneyin.';
            setErrors(newErrors);
            return false;
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // cPanel e-posta ile doğrudan e-posta gönderimi
            const emailBody = `
İsim: ${formData.name}
...
---
Bu mesaj 2B Global Enerji web sitesinden gönderilmiştir.
            `.trim();

            // mailto link ile e-posta uygulamasını aç
            const mailtoLink = `mailto:${process.env.NEXT_PUBLIC_CONTACT_EMAIL || 'info@2bltd.com.tr'}?subject=${encodeURIComponent(`İletişim Formu: ${formData.subject}`)}&body=${encodeURIComponent(emailBody)}`;
    ...
    return (
        <div className="min-h-screen bg-[#f8f8ff] relative">
            <div className="container mx-auto px-4 py-8 pb-2 sm:pb-16 relative z-10">
                            ...
                            >
                                ...
                                {errors.rateLimit && (
                                    <div className="mt-4 p-4 bg-yellow-100 border border-yellow-400 text-yellow-700 rounded-lg">
                                        <p className="font-medium">Çok sık mesaj gönderiyorsunuz</p>
                                        <p className="text-sm">{errors.rateLimit}</p>
                                    </div>
                                )}
